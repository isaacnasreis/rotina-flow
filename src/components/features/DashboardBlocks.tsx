    ...
    <div>
      <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
        <button 
          ...
          className={clsx(
            "px-6 py-2 rounded-full text-sm font-black uppercase tracking-widest transition-all whitespace-nowrap", 
        ...
        {blocks.map(b => (
          ...
          >
            <button onClick={() => setActiveTab(b.id)}>{b.name}</button>
            <button 
              onClick={(e) => {
                e.stopPropagation();
                setBlockToDelete(b.id);
              }} 
              className="opacity-0 group-hover:opacity-100 hover:text-red-300 transition-opacity p-1"
            >
              <X size={14} />
            </button>
          </div>
        ...
        {isCreatingBlock ? (
          ...
          >
            <input 
              ...
              type="text" 
              name="name" 
              className="bg-white/10 px-4 py-2 rounded-full text-sm font-bold outline-none w-32 placeholder:text-white/20 focus:ring-1 focus:ring-purple-500 transition-all" 
          ...
          <button 
            ...
            className="px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 text-white/40 transition-all flex items-center justify-center ml-2 border border-dashed border-white/20 hover:border-white/50"
          >
            <Plus size={16}/>
          </button>
        )}
